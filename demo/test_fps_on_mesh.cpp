//
// FPS seed visualization demo.
// Loads a mesh, computes initial seeds via farthest-point sampling (FPS),
// runs a single BFS from those seeds, and visualizes the initial Voronoi
// partition together with the seed positions.
//
...
struct CLIArgs
{
    ...
    {
        CLI::App app{"FPS Seed Visualization"};
...
int main(int argc, char* argv[])
{
    ...
    // ── Vertex positions for FPS ─────────────────────────────────────
    std::vector<std::tuple<double,double,double>> vertex_positions(num_vertices);
    for (int i = 0; i < num_vertices; i++)
        vertex_positions[i] = {V(i,0), V(i,1), V(i,2)};

    // ── Initialize seeds via FPS ─────────────────────────────────────
    LloydOptions opts;
    opts.seed_selection_method = LloydOptions::SeedSelectionMethod::FPS;

    std::vector<int> fps_seeds;
    auto t0 = std::chrono::high_resolution_clock::now();
    Lloyd::initialize_seeds_FPS_based(
        Gp.data(), Gi.data(), num_vertices,
        vertex_to_cc, num_vertices_per_cc, vertex_positions,
        patch_size, opts, fps_seeds);
    auto t1 = std::chrono::high_resolution_clock::now();
    double fps_ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
    spdlog::info("FPS seeds: {} seeds in {:.2f} ms", fps_seeds.size(), fps_ms);

    // ── Single BFS from FPS seeds to get initial partition ───────────
    std::vector<int> node_to_cluster(num_vertices, -1);
    std::vector<int> dist(num_vertices, -1);
    for (int i = 0; i < static_cast<int>(fps_seeds.size()); i++) {
        node_to_cluster[fps_seeds[i]] = i;
        dist[fps_seeds[i]] = 0;
    }
    Lloyd::multi_source_bfs(Gp.data(), Gi.data(), num_vertices,
                            fps_seeds, node_to_cluster, dist);

    int num_clusters = static_cast<int>(fps_seeds.size());
    ...
    polyscope::options::programName = "FPS Seed Viewer";
    polyscope::init();
    ...
    auto* pc = polyscope::registerPointCloud("fps seeds", seed_pos);
    ...
    polyscope::state::userCallback = []() {
        ImGui::Begin("FPS Seed -- Initial Partition Stats");
